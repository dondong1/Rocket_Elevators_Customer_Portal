...
currentCustomer.open('GET',  `https://donnguyen.herokuapp.com/api/Customers/verify/${email}`);
...
setTimeout(function () {
    ...
    getBld.open('GET', `https://donnguyen.herokuapp.com/api/Buildings/CustomerId/${customersId}`);
    getBld.send();
...
$("#select_build").on('click change', function(){
    ...
    if(buildID !== 'null'){
        getBat.open('GET', `https://donnguyen.herokuapp.com/api/Batteries/buildingId/${buildingId}`);
        getBat.send();
    }
});
...
$("#select_bat").on('click change', function(){
   ...
   //Send AJAX request only if value is not null
    if(batteryID !== 'null'){
        getCol.open('GET', `https://donnguyen.herokuapp.com/api/Columns/batteriesId/${batteriesId}`);
        getCol.send();
    }
...
$("#select_col").on('change click', function(){
   ...
   if (getElev.readyState === 4){ 
    ...
    //Send AJAX request only if value is not null
    if(columnID !== 'null'){
        getElev.open('GET', `https://donnguyen.herokuapp.com/api/Columns/batteriesId/${batteriesId}`);
        getElev.send();
    }
